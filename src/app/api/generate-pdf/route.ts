import { NextResponse } from "next/server";
import { generatePDF } from "@/lib/pdf-generator";
import {
  buildSubtitle,
  generateResultSections,
  inputFromMetadata,
} from "@/lib/result-sections";
import { getPayment } from "@/lib/yukassa";
import { SITE_NAME } from "@/lib/site-name";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * Builds the paid PDF for the download button on /thank-you.
 *
 * The reading is the product, so the request has to prove it belongs to a
 * completed payment: the browser sends the payment id it got back from
 * /api/checkout, and the name and topic are then read from that payment's
 * metadata rather than from the request body. An unverified endpoint here would
 * hand the paid reading to anyone who posts a name, and trusting the posted
 * body would let a buyer of the cheapest tier request the fullest reading.
 */
export async function POST(request: Request) {
  let body: { paymentId?: unknown };

  try {
    body = (await request.json()) as { paymentId?: unknown };
  } catch {
    return NextResponse.json({ error: "Некорректный запрос." }, { status: 400 });
  }

  const paymentId = typeof body.paymentId === "string" ? body.paymentId.trim() : "";

  if (!paymentId) {
    return NextResponse.json(
      { error: "Не указан платёж. Открой страницу заказа заново." },
      { status: 400 },
    );
  }

  let payment;
  try {
    payment = await getPayment(paymentId);
  } catch (error) {
    console.error("[generate-pdf] payment lookup failed", {
      paymentId,
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json({ error: "Не удалось проверить оплату." }, { status: 502 });
  }

  if (!payment || payment.status !== "succeeded") {
    return NextResponse.json(
      { error: "Оплата по этому заказу не найдена." },
      { status: 403 },
    );
  }

  const input = inputFromMetadata(payment.metadata);

  if (!input) {
    return NextResponse.json({ error: "В заказе нет данных расклада." }, { status: 422 });
  }

  const pdfBuffer = await generatePDF({
    title: "Твой рунический расклад",
    userName: input.name,
    subtitle: buildSubtitle(input),
    sections: generateResultSections(input, payment.metadata.plan),
    siteName: SITE_NAME,
  });

  return new NextResponse(new Uint8Array(pdfBuffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": 'attachment; filename="runy.pdf"',
      "Content-Length": String(pdfBuffer.length),
      "Cache-Control": "no-store",
    },
  });
}
