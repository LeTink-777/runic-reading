...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #111318; color: #E8E6E1;">
        <h1 style="color: #E8820C; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
        ...
        ${resultHtml}
        <hr style="border: 1px solid #1C1F26; margin: 30px 0;" />
